...
@section('title', 'Login - ProjectPals')
@section('meta_description', 'Masuk ke ProjectPals untuk mengelola room kamu.')
		...
		<div class="space-y-6">
			...
			<div class="space-y-4">
				<div class="inline-flex items-center gap-2 rounded-full border px-3 py-1" style="background-color: var(--color-surface-container-high); border-color: rgba(60,73,78,0.2);">
					...
					<span class="text-xs uppercase tracking-[0.3em]" style="color: var(--color-on-surface-variant);">Secure Access</span>
				</div>
				<h1 class="text-4xl md:text-5xl font-bold" style="color: var(--color-on-surface);">Welcome back, pal.</h1>
				<p class="max-w-md text-base" style="color: var(--color-on-surface-variant);">
					Masuk untuk melanjutkan kendali room dan kolaborasi tim kamu.
				</p>
			</div>
			<div class="rounded-2xl border p-6" style="border-color: rgba(60,73,78,0.2); background: linear-gradient(140deg, rgba(168,232,255,0.1), rgba(80,255,48,0.08));">
				<p class="text-sm uppercase tracking-[0.3em]" style="color: var(--color-tertiary);">Tip</p>
				<p class="mt-2 text-sm" style="color: var(--color-on-surface-variant);">
					Gunakan username yang kamu buat saat registrasi untuk login.
				</p>
			</div>
		</div>

		<div class="auth-card rounded-2xl border p-6 md:p-8">
			<h2 class="text-2xl font-semibold" style="color: var(--color-on-surface);">Login</h2>
			<p class="mt-2 text-sm" style="color: var(--color-on-surface-variant);">Masukkan username dan password kamu.</p>
			...
			<form method="POST" action="{{ route('auth.login') }}" class="mt-6 space-y-4">
				...
				<div>
					...
					<input id="username" name="username" type="text" value="{{ old('username') }}" required class="w-full rounded-md border px-3 py-2" style="background-color: var(--color-surface-container-high); border-color: var(--color-outline-variant); color: var(--color-on-surface);" placeholder="contoh: projectpal" />
				</div>
				<div>
				...
				<div class="flex items-center justify-between text-sm">
					<label class="inline-flex items-center gap-2" style="color: var(--color-on-surface-variant);">
						<input type="checkbox" name="remember" class="rounded" />
						Remember me
					</label>
					<a href="{{ route('register') }}" class="hover:underline" style="color: var(--color-primary-container);">Buat akun baru</a>
				</div>
				<button type="submit" class="w-full rounded-md px-4 py-2 font-semibold" style="background-color: var(--color-primary-container); color: var(--color-on-primary);">Login</button>
			</form>
		</div>
	</div>
</section>
